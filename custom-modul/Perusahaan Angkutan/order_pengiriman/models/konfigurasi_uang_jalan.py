...
class KonfigurasiUangJalan(models.Model):
    ...
    uang_jalan = fields.Float('Uang Jalan', compute='_calculate_uang_jalan', store=True, readonly=False)
    manual_uang_jalan = fields.Float('Manual Uang Jalan')
    ...
    @api.depends('uang_makan', 'uang_solar', 'kuli', 'tol', 'tonase', 'lain_lain', 'aktifkan_pembulatan', 'manual_uang_jalan')
    def _calculate_uang_jalan(self):
        for record in self:
            if record.manual_uang_jalan:
                record.uang_jalan = record.manual_uang_jalan
            else:
                computed_value = record.uang_makan + record.uang_solar + record.kuli + record.tol + record.tonase + record.lain_lain
                if record.aktifkan_pembulatan:
